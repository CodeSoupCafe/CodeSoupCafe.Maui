...
public class ItemGalleryViewModel : NotifyPropertyChanged, IDisposable
{
  ...
  private IEnumerable<ISortable>? sourceCollection;
  ...
  public List<string> SortOrders { get; } = ["Ascending", "Descending"];
  public List<string> SortProperties { get; } = ["DateCreated", "DateUpdated", "Title"];
  ...
  public void SetItemsSource(IEnumerable<ISortable> items)
  {
    ...
    if (sourceCollection is INotifyCollectionChanged newNotify)
    {
  ...
  {
    var source = sourceCollection?.ToList() ?? new List<ISortable>();
    var filtered = source;
